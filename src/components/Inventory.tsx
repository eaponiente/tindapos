...
export default function Inventory({ items, categories, reloadItems, employee }: InventoryProps) {
    ...
    const render = () => {
      openModal(
        <>
          ...
          <footer>
            {!isNew && (
              <button className="btn danger" onClick={() => deleteConfirmModal(item)}>
                Delete
              </button>
            )}
  ...
  function deleteConfirmModal(item: Item) {
    let pin = '';
    let error = '';
    ...
    const render = () => {
      openModal(
        <>
          ...
          <div className="bodyPad">
            <p>
              Delete <b>{item.name}</b>? This can&apos;t be undone. Enter your PIN to confirm.
            </p>
            <div className="field">
  ...
  function adjustModal(item: Item) {
    let reason: AdjustReason = 'receive';
    let qty = '';
    let error = '';
    const render = () => {
      openModal(
        <>
          ...
          <div className="bodyPad">
            ...
            <div className="field" style={{ marginTop: 12 }}>
              <label>Reason</label>
              <select defaultValue={reason} onChange={(e) => (reason = e.target.value as AdjustReason)}>
  ...
  return (
    <section className="screen">
      <div className="topbar">
        ...
        />
        <button className="btn primary" onClick={() => itemModal(null)}>
          + New item
        </button>
      </div>
      {stats && (
        <div className="statRow">
          ...
          <div className="stat">
            ...
            <div className="val" style={{ color: stats.low ? 'var(--danger)' : 'inherit' }}>
              {stats.low}
            </div>
          </div>
          <div className="stat">
            <div className="lbl">Out of stock</div>
            <div className="val">{stats.out}</div>
          </div>
          <div className="stat">
            <div className="lbl">Stock value (cost)</div>
            <div className="val">{peso(stats.stock_value)}</div>
          </div>
        </div>
      )}
            ...
            {filtered.map((i) => (
              <tr key={i.id}>
                ...
                <td style={{ whiteSpace: 'nowrap' }}>
                  <button className="btn small" onClick={() => adjustModal(i)}>
                    ± Stock
                  </button>{' '}
                  <button className="btn small" onClick={() => itemModal(i)}>
                    Edit
                  </button>
                </td>
              </tr>
