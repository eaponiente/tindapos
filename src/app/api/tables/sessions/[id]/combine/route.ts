...
/** COMBINE TABLES — add more free tables to this open session. */
export const POST = handler(async (request: NextRequest, { params }: Ctx) => {
  ...
  if (tableIds.length === 0) return fail('Select at least one table to combine');

  const { error } = await db().rpc('combine_tables', {
